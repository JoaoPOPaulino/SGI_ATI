...
const StatusBadge: React.FC<StatusBadgeProps> = ({ type, value }) => {
  if (type === 'status') {
    ...
    switch (status) {
      ...
      case 'GUARDADO': return <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 ${S} font-semibold rounded-full bg-sky-950/50 border border-sky-500/30 text-sky-400`}><span className="w-1.5 h-1.5 rounded-full bg-sky-400" />Pronto</span>;
