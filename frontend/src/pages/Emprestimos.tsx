...
const Emprestimos: React.FC = () => {
  ...
  const itensAlocaveisParaEvento = useMemo(() => {
    ...
    return itensDisponiveis.filter(
      (i) => !evento.itens_alocados.includes(i.id),
    );
  ...
  const handleCreateLoan = async (e: React.FormEvent) => {
    ...
    if (isEstagiario) {
      setFormLoanError(
        "Apenas Técnicos e Perfis Superiores podem efetivar empréstimos.",
      );
      setIsSaving(false);
      return;
    }
    ...
    try {
      ...
      await createMovimentacao({
        ...
        destino: `Empréstimo: ${formResponsavel}`,
  ...
  const handleCreateEvent = async (e: React.FormEvent) => {
    ...
    if (isEstagiario) {
      setFormEventError("Apenas Técnicos e Superiores podem criar eventos.");
      setIsSaving(false);
      return;
    }
    ...
    try {
      const newEvent: Evento = {
        ...
        itens_alocados: formItensSelecionados,
      };
        ...
        for (const itemId of formItensSelecionados) {
          ...
          if (it) {
            await createMovimentacao({
              ...
              item_nome: it.nome,
              tipo: "TRANSFERENCIA",
              origem: it.localizacao_atual,
              destino: `Evento: ${formNomeEvento}`,
              solicitante_id: user?.id || "",
              solicitante_nome: user?.nome || "",
              status_aprovacao: "APROVADO",
              data_movimentacao: new Date().toISOString(),
      ...
      setFormEventSuccess(
        formItensSelecionados.length > 0
  ...
  const eventoSelecionado = manageEventId
    ? eventos.find((e) => e.id === manageEventId)
    : null;
  const itensDoEvento = eventoSelecionado
    ? itens.filter((i) => eventoSelecionado.itens_alocados.includes(i.id))
    : [];

  const loansAtivos = useMemo(
    () => loans.filter((l) => l.status === "ATIVO"),
    [loans],
  );
  const today = new Date();
    ...
    <div className="space-y-8 animate-fade-in text-on-surface font-body">
      <div>
        ...
        <p className="text-xs text-outline font-semibold">
          Gerencie saídas temporárias de equipamentos e organize alocações para
          eventos.
        </p>
      </div>
      ...
      {formLoanSuccess && (
        <div
          className="p-3 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center gap-2 text-xs text-emerald-700 animate-slide-up"
          role="status"
        >
          <Check size={14} className="shrink-0" /> {formLoanSuccess}
          <button
            onClick={() => setFormLoanSuccess("")}
          ...
          <div className="glass-panel w-full max-w-md rounded-2xl p-8 shadow-2xl border border-outline-variant/10 bg-surface-container-lowest animate-slide-up">
            ...
            <form onSubmit={handleReturnItem} className="space-y-5">
              ...
              <div>
                ...
                <select
                  id="return-condicao"
                  value={returnCondicao}
                  onChange={(e) =>
                    setReturnCondicao(e.target.value as CondicaoItem)
                  }
                  className="w-full px-3 py-2 bg-surface border border-outline rounded-xl text-xs focus:ring-1 focus:ring-primary text-on-surface"
                >
              ...
              <div className="pt-4 border-t border-outline-variant/20 flex justify-end gap-3">
                ...
                <button
                  type="submit"
                  className="px-5 py-2.5 custom-gradient-btn text-white text-xs font-semibold rounded-xl shadow-md active:scale-95"
                >
