...
import { fetchAllMovimentacoes, createMovimentacao, updateMovimentacao } from '../services/movimentacoesService';
import { fetchLaudos } from '../services/laudosService';
import { createAssinaturaGuia } from '../services/assinaturasService';
          ...
          {canModify && (
            <div className="glass-panel p-5 rounded-2xl border border-outline-variant/10 bg-surface-container-lowest">
              ...
              <form onSubmit={handleRequestDecommission} className="space-y-3.5">
                ...
                <div><label className={`block ${lbl} font-bold text-outline uppercase tracking-wider mb-1`}>Justificativa Técnica</label>                <textarea rows={2} value={formMotivoBaixa} onChange={(e) => setFormMotivoBaixa(e.target.value)} maxLength={500} placeholder="Descreva o defeito sem conserto, obsolescência ou perda patrimonial..." className="w-full px-3 py-2 bg-surface border border-outline rounded-lg text-on-surface placeholder:text-outline text-[11px] focus:outline-none resize-none" /></div>
