...
const userSchema = z.object({
  ...
  cpf: z
    .string()
    .refine((v) => isValidCpf(v), { message: 'CPF inválido. Informe 11 dígitos.' }),
  perfil: z.enum(['ESTAGIARIO', 'TECNICO', 'SUPERIOR', 'ADMIN'] as const),
  polo: z.string().optional()
});

const POLOS_DISPONIVEIS = [
  'GSM',
  'Laboratório'
];
...
const Admin: React.FC = () => {
  ...
  const [formErrors, setFormErrors] = useState<{ nome?: string; email?: string; cpf?: string; general?: string }>({});
  ...
  const [selectedUser, setSelectedUser] = useState<SupabaseUsuario | null>(null);
  ...
  const filteredUsers = useMemo(() => {
    ...
    if (searchTerm.trim()) {
      ...
      result = result.filter(
        u => u.nome.toLowerCase().includes(term) || u.email.toLowerCase().includes(term) || u.cpf.includes(term)
      );
    }
    ...
    result.sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case 'perfil': {
          const hierarchy: Record<PerfilUsuario, number> = { ADMIN: 4, SUPERIOR: 3, TECNICO: 2, ESTAGIARIO: 1 };
          return multiplier * ((hierarchy[a.perfil] || 0) - (hierarchy[b.perfil] || 0));
        }
        case 'polo':
          return multiplier * (a.polo || '').localeCompare(b.polo || '', 'pt-BR');
        case 'status':
  ...
  const isLastActiveAdmin = (userId: string): boolean => {
    return adminsAtivos.length <= 1 && adminsAtivos.some(a => a.id === userId);
  };
  ...
  const requestConfirm = (
    ...
    setConfirm({ open: true, title, message, variant, confirmLabel, onConfirm });
  };
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    if (!result.success) {
      const fieldErrors: { nome?: string; email?: string; cpf?: string } = {};
    ...
    if (!response.success) {
      setFormErrors({ general: response.error || 'Erro ao cadastrar usuário.' });
    ...
    setPolo('');
    setSuccessMsg(`Usuário "${nome.trim()}" cadastrado com sucesso! Um e-mail de boas-vindas foi enviado.`);
    setIsSubmitting(false);
    await loadUsuarios();
  };
  ...
  const changeUserRole = (id: string, newRole: PerfilUsuario) => {
    ...
    const isLastAdminDowngrade = oldRole === 'ADMIN' && newRole !== 'ADMIN' && isLastActiveAdmin(id);
  ...
  const executeChangeRole = async (id: string, newRole: PerfilUsuario, oldRole: PerfilUsuario) => {
  ...
  const SortIcon = ({ field }: { field: SortField }) => {
    if (sortField !== field) return <ArrowUpDown size={12} className="text-outline/50" />;
  ...
  return (
    <div className="space-y-8 animate-fade-in text-on-surface font-body">
      <div>
        <h1 className="text-3xl font-extrabold tracking-tight text-primary">Painel Administrativo</h1>
        <p className="text-xs text-outline font-semibold">Gerencie usuários, permissões, e alocações hierárquicas da plataforma.</p>
      </div>
      ...
      {successMsg && (
        <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center gap-2 text-xs text-emerald-700 animate-slide-up">
          ...
          <p>{successMsg}</p>
          <button onClick={() => setSuccessMsg('')} className="ml-auto text-emerald-400 hover:text-emerald-600">&times;</button>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="glass-panel p-5 rounded-2xl border border-outline-variant/10 bg-surface-container-lowest flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-black text-outline uppercase tracking-wider">Total de Usuários</p>
        ...
        <div className="glass-panel p-5 rounded-2xl border border-outline-variant/10 bg-surface-container-lowest flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-black text-outline uppercase tracking-wider">Contas Ativas</p>
        ...
        <div className="glass-panel p-5 rounded-2xl border border-outline-variant/10 bg-surface-container-lowest flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-black text-outline uppercase tracking-wider">Contas Inativas</p>
            <p className="text-2xl font-bold text-on-surface">{stats.inativos}</p>
          </div>
        </div>
        ...
        <div className="glass-panel p-5 rounded-2xl border border-outline-variant/10 bg-surface-container-lowest flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-black text-outline uppercase tracking-wider">Administradores</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="glass-panel p-6 rounded-2xl border border-outline-variant/10 bg-surface-container-lowest h-fit">
          ...
          <form onSubmit={handleCreateUser} className="space-y-4">
            ...
            <div>
              <label className="block text-[10px] font-black text-outline uppercase tracking-wider mb-1.5">Nome Completo</label>
              <input
                type="text"
                value={nome}
              ...
              />
              {formErrors.nome && <p className="mt-1.5 text-[10px] text-red-500 font-semibold">{formErrors.nome}</p>}
            </div>

            <div>
              <label className="block text-[10px] font-black text-outline uppercase tracking-wider mb-1.5">E-mail Corporativo</label>
              <input
                type="email"
                value={email}
              ...
              />
              {formErrors.email && <p className="mt-1.5 text-[10px] text-red-500 font-semibold">{formErrors.email}</p>}
            </div>

            <div>
              <label className="block text-[10px] font-black text-outline uppercase tracking-wider mb-1.5">CPF (Login)</label>
              <input
                type="text"
                inputMode="numeric"
                value={cpf}
              ...
              />
              {formErrors.cpf && <p className="mt-1.5 text-[10px] text-red-500 font-semibold">{formErrors.cpf}</p>}
            </div>

            <div>
              <label className="block text-[10px] font-black text-outline uppercase tracking-wider mb-1.5">Nível de Acesso (Perfil)</label>
              <select
                value={perfil}
              ...
              >
                <option value="ESTAGIARIO">Estagiário (Apenas Visualização e Solicitações)</option>
                <option value="TECNICO">Técnico (Pode aprovar e dar baixa em manutenções)</option>
                <option value="SUPERIOR">Superior (Aprova baixas definitivas)</option>
                <option value="ADMIN">Administrador (Controle Total)</option>
              </select>
            </div>

            <div>
              <label className="block text-[10px] font-black text-outline uppercase tracking-wider mb-1.5">Alocação Base (Polo)</label>
              <select
                value={polo}
            ...
            >
              {isSubmitting ? (
                <><Loader2 size={16} className="animate-spin" /> Enviando convite...</>
              ) : (
          ...
          <div className="glass-panel p-6 rounded-2xl border border-outline-variant/10 bg-surface-container-lowest flex flex-col h-full">
                ...
                <table className="w-full text-left border-collapse min-w-[680px]">
                  <thead>
                    <tr className="border-b border-outline-variant/20">
                      ...
                      <th className="py-3 px-4 text-[10px] font-black text-outline uppercase tracking-wider text-right">Ações</th>
                    </tr>
                  </thead>
                    ...
                    {filteredUsers.map(u => (
                      ...
                      >
                          ...
                          >
                            ...
                            <div>
                              <p className="text-xs font-bold text-on-surface flex items-center gap-2 max-w-[180px]">
                                ...
                                {u.id === currentUser?.id && (
                                  <span className="text-[9px] bg-primary/10 text-primary px-1.5 py-0.5 rounded font-black uppercase tracking-wider">Você</span>
                                )}
                              </p>
                              <p className="text-[10px] text-outline font-medium truncate">{u.email}</p>
                        ...
                        <td className="py-3 px-4">
                          <select
                            value={u.perfil}
                            onChange={(e) => changeUserRole(u.id, e.target.value as PerfilUsuario)}
                            disabled={!u.ativo}
                        ...
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-1 bg-surface-container px-2 py-0.5 rounded text-[9px] font-semibold text-on-surface-variant border border-outline-variant/10">
                            ...
                            <select
                              value={u.polo || ''}
                              onChange={(e) => changeUserPolo(u.id, e.target.value)}
                              disabled={!u.ativo}
            ...
            <div className="mt-4 p-4 bg-primary-fixed/20 border border-primary/10 rounded-xl flex items-start gap-3">
              ...
              <div className="text-[10px] text-primary/80 leading-relaxed">
                <strong className="block text-primary mb-1 text-xs">Políticas de Acesso:</strong>
                O <strong>Estagiário</strong> só visualiza e solicita. O <strong>Técnico</strong> pode aprovar manutenções. O <strong>Superior</strong> é necessário para aprovar Baixas Definitivas de Patrimônio. O <strong>Administrador</strong> controla permissões, filiais (polos) e tem visibilidade total do SGI-ATI.
                <br />
                <span className="text-[9px] text-primary/60 mt-1 inline-block">
                  Todas as ações administrativas são registradas no log de auditoria.
                </span>
              </div>
            </div>
