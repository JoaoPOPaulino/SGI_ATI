...
const Movimentacoes: React.FC = () => {
  ...
  // Estados de Paginação (aba Consultar)
  ...
  const saveAssinatura = async () => {
    ...
    if (!signingNome.trim()) { alert("Informe o nome do assinante."); return; }
    if (!signingAssinatura) { alert("Realize a assinatura na caixa de assinatura."); return; }
    ...
    const saved = await createAssinaturaGuia({
      ...
      localizacao: signingLocalizacao.trim() || signingMov.local_retirada || signingMov.origem,
    ...
    if (!saved) { alert("Erro ao salvar assinatura. Tente novamente."); return; }
    ...
    setSigningMov(null);
    await loadData();
    if (activeGuia?.id === signedMovId) await reloadAssinaturas(signedMovId);
  };
  ...
  const handleRequest = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!item) { setFormError("Equipamento não encontrado ou indisponível."); setIsSaving(false); return; }
  ...
  const handleExportMovimentacoesCsv = () => {
    ...
    const headers = ["ID", "Chamado", "Patrimônio", "Série", "Equipamento", "Tipo", "Origem", "Destino", "Solicitante", "Status", "Status Guia", "Data"];
    const rows = data.map((m) => [m.id, m.chamado || "", m.item_patrimonio || "", m.item_numero_serie || "", m.item_nome, m.tipo, m.origem, m.destino, m.solicitante_nome, m.status_aprovacao, m.status_guia || "", m.data_movimentacao]);
      ...
      <div className="flex items-center justify-between px-4 py-2.5 bg-surface-container-low rounded-xl border border-outline-variant/20 flex-wrap gap-2">
        ...
        <div className="flex items-center gap-1">
          ...
          {paginas.map((item, idx) =>
            item === "..." ? (
              <span key={`ellipsis-${idx}`} className="w-[30px] h-[30px] flex items-center justify-center text-xs font-bold text-outline select-none">…</span>
            ) : (
          ...
          <div className="glass-panel p-6 rounded-2xl border border-outline-variant/10 bg-surface-container-lowest shadow-sm">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-5 border-b border-outline-variant/10 pb-3">
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <button type="button" onClick={handleExportMovimentacoesCsv}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-fade-in">
          <div className="bg-surface-container-lowest w-full max-w-2xl rounded-2xl p-6 shadow-2xl border border-outline-variant/10 animate-slide-up max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
              <div>
                <label htmlFor="signingNome" className="block text-[10px] font-black text-outline uppercase tracking-wider mb-1.5">Nome do Assinante *</label>
                <input id="signingNome" type="text" value={signingNome} onChange={(e) => setSigningNome(e.target.value)} className="w-full px-3 py-2 bg-surface border border-outline rounded-xl text-xs text-on-surface" />
              </div>
              <div>
                <label htmlFor="signingCpf" className="block text-[10px] font-black text-outline uppercase tracking-wider mb-1.5">CPF</label>
                <input id="signingCpf" type="text" value={signingCpf} onChange={(e) => setSigningCpf(e.target.value)} className="w-full px-3 py-2 bg-surface border border-outline rounded-xl text-xs text-on-surface" />
              </div>
              <div className="sm:col-span-2">
                <label htmlFor="signingLocalizacao" className="block text-[10px] font-black text-outline uppercase tracking-wider mb-1.5">Localização</label>
                <input id="signingLocalizacao" type="text" value={signingLocalizacao} onChange={(e) => setSigningLocalizacao(e.target.value)} className="w-full px-3 py-2 bg-surface border border-outline rounded-xl text-xs text-on-surface" />
              </div>
              <div className="sm:col-span-2">
                <label htmlFor="signingObservacao" className="block text-[10px] font-black text-outline uppercase tracking-wider mb-1.5">Observação</label>
                <textarea id="signingObservacao" rows={2} value={signingObservacao} onChange={(e) => setSigningObservacao(e.target.value)} className="w-full px-3 py-2 bg-surface border border-outline rounded-xl text-xs text-on-surface" />
              </div>
            </div>
